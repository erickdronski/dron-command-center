...
const categoryConfig: Record<Category, { color: string; bg: string; border: string; activeBg: string; icon: React.ReactNode; label: string }> = {
  ESM: { color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20', activeBg: 'bg-blue-500/20', icon: <Server size={16} />, label: 'Enterprise Service Management' },
  UEM: { color: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/20', activeBg: 'bg-green-500/20', icon: <Monitor size={16} />, label: 'Unified Endpoint Management' },
  Security: { color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20', activeBg: 'bg-red-500/20', icon: <Shield size={16} />, label: 'Security' },
};

const typeLabels: Record<ProductType, { label: string; color: string }> = {
  saas: { label: 'SaaS', color: 'bg-blue-500/20 text-blue-400' },
  'on-prem': { label: 'On-Prem', color: 'bg-amber-500/20 text-amber-400' },
  hybrid: { label: 'Hybrid', color: 'bg-purple-500/20 text-purple-400' },
  'add-on': { label: 'Add-On', color: 'bg-white/5 text-[#999]' },
  package: { label: 'Package', color: 'bg-green-500/20 text-green-400' },
};
...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className={`bg-[#111] border border-[#222] rounded-xl overflow-hidden  transition-all ${expanded ? '' : 'hover:border-[#333]'}`}>
      <button onClick={() => setExpanded(!expanded)} className="w-full text-left p-4 flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[#666] mt-1 line-clamp-2">{product.shortDescription}</p>
        </div>
        {expanded ? <ChevronDown size={16} className="text-[#555] mt-1 flex-shrink-0" /> : <ChevronRight size={16} className="text-[#555] mt-1 flex-shrink-0" />}
      </button>

      {expanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-[#222] pt-4">
          <div>
            <h4 className="text-[10px] font-semibold text-[#555] uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <Layers size={12} className="text-purple-400" /> Key Capabilities
            </h4>
          ...
          <div>
            <h4 className="text-[10px] font-semibold text-[#555] uppercase tracking-wider mb-1 flex items-center gap-1.5">
              <Users size={12} className="text-purple-400" /> Target Audience
            </h4>
            <p className="text-xs text-[#999]">{product.targetAudience}</p>
          </div>

          <div>
            <h4 className="text-[10px] font-semibold text-[#555] uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <HelpCircle size={12} className="text-purple-400" /> Discovery Questions
            </h4>
          ...
          <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-3">
            <h4 className="text-[10px] font-semibold text-[#555] uppercase tracking-wider mb-1 flex items-center gap-1.5">
              <Key size={12} className="text-purple-400" /> Licensing
            </h4>
            <p className="text-xs text-[#999]">{product.licensing}</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className="text-center px-4 py-2 bg-purple-500/10 border border-purple-500/20 rounded-lg">
          <div className="text-lg font-bold text-purple-400">{filtered.length}</div>
          <div className="text-[10px] text-[#555]">Total Products</div>
        </div>
        {(['ESM', 'UEM', 'Security'] as Category[]).map(cat => {
          ...
          return (
            <div key={cat} className={`text-center px-4 py-2 ${cfg.bg} border ${cfg.border} rounded-lg`}>
              ...
              <div className="text-[10px] text-[#555]">{cat}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-[#111] border border-[#222] rounded-lg text-sm text-white placeholder-[#444] focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/20"
        />
      </div>
          ...
          return (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors border-b-2 ${
                isActive ? `${cfg.color} ${cfg.activeBg} border-current` : 'text-[#555] border-transparent hover:text-[#ccc]'
              }`}>
              {cfg.icon}
              <span>{cat}</span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${isActive ? cfg.bg + ' ' + cfg.color : 'bg-white/5 text-[#555]'}`}>{counts[cat]}</span>
            </button>
          );
        })}
      </div>
      ...
      {/* Product Description */}
      <div className={`text-xs text-[#666] ${categoryConfig[activeCategory].bg} border ${categoryConfig[activeCategory].border} rounded-lg px-4 py-2`}>
      ...
      {categoryProducts.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-[#555]">No products match your search in {activeCategory}.</p>
        </div>
      ) : (
